...
module.exports.editarNoticias = function(application, req, res){
    var noticia = req.body;
    var connection = application.config.dbConnection();
    var noticiasModel = new application.app.models.noticiasDAO(connection);
    res.render('./noticias/form_add_noticias', {validacao : erros, noticia : noticia});
    ...
    if (erros) {
        console.log(erros);
        res.render('./noticias/editar_noticias', {validacao : erros, noticia : noticia});
        return;
    }
    noticiasModel.editarNoticias(noticia, function(error, result) {
        //tratar o F5 (refresh), evitar duplicidades, redirecionar para pagina correta
        res.redirect('/noticias');
    });
}